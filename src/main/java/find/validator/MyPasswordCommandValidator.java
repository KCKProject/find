...
public class MyPasswordCommandValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		System.out.println("입력 비번 : "+command.getUserPasswordCurrent());
		System.out.println("DB 비번 : "+command.getUserPasswordCurrentChk());
		...
		try {
			current_sha = SHA256Util.SHA256Encrypt(current);
		} catch (Exception e) {			
			e.printStackTrace();
		}
		...
		try {
			pwdNew_sha = SHA256Util.SHA256Encrypt(pwdNew);
		} catch (Exception e) {
			e.printStackTrace();
		}
